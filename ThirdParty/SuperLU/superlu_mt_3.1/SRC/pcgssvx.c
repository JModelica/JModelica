...
void pcgssvx(int_t nprocs, superlumt_options_t *superlumt_options,
             SuperMatrix *A, int_t *perm_c, int_t *perm_r, equed_t *equed,
             float *R, float *C, SuperMatrix *L, SuperMatrix *U, SuperMatrix *B,
             SuperMatrix *X, float *recip_pivot_growth, float *rcond,
             float *ferr, float *berr, superlu_memusage_t *superlu_memusage,
             int_t *info) {
  /*
   ...
   *           trans = TRANS:  (diag(R)*A*diag(C))**T *inv(diag(R))*X =
   *diag(C)*B trans = CONJ:   (diag(R)*A*diag(C))**H *inv(diag(R))*X = diag(C)*B
   ...
   *matrix that usually preserves sparsity. For more details of this step, see
   *csp_colorder.c.
   *
   ...
   *           User-supplied work space and space for the output data
   *structures. Not referenced if lwork = 0;
   *
   *         o lwork (int_t)
   ...
   *diag(C). = BOTH: Both row and column equilibration, i.e., A was replaced by
   *diag(R)*A*diag(C). If superlumt_options->fact = FACTORED, equed is an input
   *argument, otherwise it is an output argument.
   *
  ...
  *info = 0;
    ...
    if (*info == 0) {
      if (B->ncol < 0 || Bstore->lda < SUPERLU_MAX(0, A->nrow) ||
        ...
        *info = -11;
               ...
               B->ncol != X->ncol || X->Stype != SLU_DN || X->Dtype != SLU_C ||
               X->Mtype != SLU_GE)
        *info = -12;
    }
  }
  ...
  if (A->Stype == SLU_NR) {
    ...
    cCreate_CompCol_Matrix(AA, A->ncol, A->nrow, Astore->nnz, Astore->nzval,
  ...
  if (dofact || equil) {
    ...
    t0 = SuperLU_timer_();
    csp_colorder(AA, perm_c, superlumt_options, &AC);
    utime[ETREE] = SuperLU_timer_() - t0;

#if (PRNTlevel >= 2)
    printf("Factor PA = LU ... relax %d\tw %d\tmaxsuper %d\trowblk %d\n", relax,
           panel_size, sp_ienv(3), sp_ienv(4));
    fflush(stdout);
#endif
  ...
  if (*info > 0) {
    ...
    *recip_pivot_growth = cPivotGrowth(A->ncol, AA, perm_c, L, U);
    ...
    /* ------------------------------------------------------------
       ...
       ------------------------------------------------------------*/
    t0 = SuperLU_timer_();
    cgsrfs(trant, AA, L, U, perm_r, perm_c, *equed, R, C, B, X, ferr, berr,
           &Gstat, info);
