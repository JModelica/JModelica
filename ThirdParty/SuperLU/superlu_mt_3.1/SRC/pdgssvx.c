...
void pdgssvx(int_t nprocs, superlumt_options_t *superlumt_options,
             SuperMatrix *A, int_t *perm_c, int_t *perm_r, equed_t *equed,
             double *R, double *C, SuperMatrix *L, SuperMatrix *U,
    ...
    if (*info == 0) {
      if (B->ncol < 0 || Bstore->lda < SUPERLU_MAX(0, A->nrow) ||
        ...
        *info = -11;
               ...
               B->ncol != X->ncol || X->Stype != SLU_DN || X->Dtype != SLU_D ||
               X->Mtype != SLU_GE)
        *info = -12;
    }
  }
  ...
  if (*info > 0) {
    ...
    *recip_pivot_growth = dPivotGrowth(A->ncol, AA, perm_c, L, U);
    ...
    /* ------------------------------------------------------------
       ...
       ------------------------------------------------------------*/
    t0 = SuperLU_timer_();
    dgsrfs(trant, AA, L, U, perm_r, perm_c, *equed, R, C, B, X, ferr, berr,
           &Gstat, info);
