...
pdgstrf_threadarg_t *pdgstrf_thread_init(SuperMatrix *A, SuperMatrix *L,
                                         SuperMatrix *U,
  ...
  *info = pdgstrf_MemInit(n, Astore->nnz, options, L, U, &Glu);
  ...
  pdgstrf_threadarg = (pdgstrf_threadarg_t *)SUPERLU_MALLOC(
      nprocs * sizeof(pdgstrf_threadarg_t));
  for (i = 0; i < nprocs; ++i) {
