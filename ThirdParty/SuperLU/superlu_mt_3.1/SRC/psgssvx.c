...
void psgssvx(int_t nprocs, superlumt_options_t *superlumt_options,
  ...
  *info = 0;
    ...
    if (*info == 0) {
      if (B->ncol < 0 || Bstore->lda < SUPERLU_MAX(0, A->nrow) ||
        ...
        *info = -11;
               ...
               B->ncol != X->ncol || X->Stype != SLU_DN || X->Dtype != SLU_S ||
               X->Mtype != SLU_GE)
        *info = -12;
    }
  }
  ...
  if (A->Stype == SLU_NR) {
    ...
    sCreate_CompCol_Matrix(AA, A->ncol, A->nrow, Astore->nnz, Astore->nzval,
  ...
  if (dofact || equil) {
    ...
    t0 = SuperLU_timer_();
    ssp_colorder(AA, perm_c, superlumt_options, &AC);
  ...
  if (*info > 0) {
    ...
    *recip_pivot_growth = sPivotGrowth(A->ncol, AA, perm_c, L, U);
    ...
    /* ------------------------------------------------------------
       ...
       ------------------------------------------------------------*/
    t0 = SuperLU_timer_();
    sgsrfs(trant, AA, L, U, perm_r, perm_c, *equed, R, C, B, X, ferr, berr,
           &Gstat, info);
