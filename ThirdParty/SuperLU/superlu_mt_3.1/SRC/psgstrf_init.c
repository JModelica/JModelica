...
void psgstrf_init(int_t nprocs, fact_t fact, trans_t trans, yes_no_t refact,
                  int_t panel_size, int_t relax, float diag_pivot_thresh,
                  yes_no_t usepr, double drop_tol, int_t *perm_c, int_t *perm_r,
                  void *work, int_t lwork, SuperMatrix *A, SuperMatrix *AC,
   ...
   *
   * fact   (input) fact_t
   *        Specifies whether or not the factored form of the matrix is
   *supplied.
   *
   * trans  (input) trans_t
  ...
  t = SuperLU_timer_();
  ssp_colorder(A, perm_c, superlumt_options, AC);
