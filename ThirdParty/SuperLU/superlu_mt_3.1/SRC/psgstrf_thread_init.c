...
psgstrf_threadarg_t *psgstrf_thread_init(SuperMatrix *A, SuperMatrix *L,
                                         SuperMatrix *U,
  ...
  pxgstrf_relax = (pxgstrf_relax_t *)SUPERLU_MALLOC((size_t)(n + 2) *
                                                    sizeof(pxgstrf_relax_t));

#if 0
    if ( options->SymmetricMode == YES ) {
        heap_relax_snode(n, options, pxgstrf_relax);
    } else {
        psgstrf_relax_snode(n, options, pxgstrf_relax);
    }
#endif
  psgstrf_relax_snode(n, options, pxgstrf_relax);

  /* Initialize mutex variables, task queue, determine panels. */
  sParallelInit(n, pxgstrf_relax, options, pxgstrf_shared);
  ...
  *info = psgstrf_MemInit(n, Astore->nnz, options, L, U, &Glu);
  ...
  psgstrf_threadarg = (psgstrf_threadarg_t *)SUPERLU_MALLOC(
      nprocs * sizeof(psgstrf_threadarg_t));
  for (i = 0; i < nprocs; ++i) {
