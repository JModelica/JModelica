...
void sp_colorder(SuperMatrix *A, int_t *perm_c, superlumt_options_t *options,
  ...
  extern void at_plus_a(const int_t, const int_t, int_t *, int_t *, int_t *,
                        int_t **, int_t **, int_t);
  ...
  if (refact == NO) {
    ...
    if (options->SymmetricMode) {
      ...
      /* Form B = A + A'. */
      at_plus_a(n, Astore->nnz, Astore->colptr, Astore->rowind, &bnz, &b_colptr,
                &b_rowind, 1);
      ...
      /* Compute etree of C. */
      sp_symetree(c_colbeg, c_colend, b_rowind, n, options->etree);
      ...
      sp_coletree(ACstore->colbeg, ACstore->colend, ACstore->rowind, A->nrow,
                  A->ncol, options->etree);
    }
    ...
    for (i = 0; i < n; ++i)
      iwork[post[i]] = post[options->etree[i]];
    for (i = 0; i < n; ++i)
      options->etree[i] = iwork[i];
    ...
    for (i = 0; i < n; ++i)
      perm_c[i] = iwork[i];
    for (i = 0; i < n; ++i)
      invp[perm_c[i]] = i; /* inverse of perm_c */
    ...
    if (options->SymmetricMode) {
...
#else
      cholnzcnt(n, b_colptr, b_rowind, invp, perm_c, options->etree,
                options->colcnt_h, &nlnz, options->part_super_h);
#endif
#if (PRNTlevel >= 1)
      printf(".. bnz %d\n", bnz);
      ...
      qrnzcnt(n, nnz, Astore->colptr, Astore->rowind, iperm, invp, perm_c,
              options->etree, options->colcnt_h, &nlnz, part_super_ata,
              options->part_super_h);
    }

#if (PRNTlevel >= 2)
    dCheckZeroDiagonal(n, ACstore->rowind, ACstore->colbeg, ACstore->colend,
                       perm_c);
...
int_t dCheckZeroDiagonal(int_t n, int_t *rowind, int_t *colbeg, int_t *colend,
                         int_t *perm) {
