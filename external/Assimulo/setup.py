...
for pg in package_arguments:
    parser.add_argument(
        "--{}-home".format(pg),
        help="Location of the {} directory".format(pg.upper()),
        type=str,
        default="",
    )
parser.add_argument("--blas-name", help="name of the blas package", default="blas")
parser.add_argument(
    "--extra-c-flags", help='Extra C-flags (a list enclosed in " ")', default=""
)
parser.add_argument(
    "--with_openmp", type="bool", help="set to true if present", default=False
)
parser.add_argument(
    "--is_static", type="bool", help="set to true if present", default=False
)
parser.add_argument(
    "--sundials-with-superlu",
    type="bool",
    help="(DEPRECATED) set to true if Sundials has been compiled with SuperLU",
    default=None,
)
parser.add_argument(
    "--debug", type="bool", help="set to true if present", default=False
)
parser.add_argument(
    "--force-32bit", type="bool", help="set to true if present", default=False
)
parser.add_argument(
    "--no-msvcr", type="bool", help="set to true if present", default=False
)
parser.add_argument(
    "--log", choices=("DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"), default="NOTSET"
)
parser.add_argument("--log_file", default=None, type=str, help="Path of a logfile")
parser.add_argument(
    "--prefix", default=None, type=str, help="Path to destination directory"
)
parser.add_argument(
    "--extra-fortran-link-flags",
    help='Extra Fortran link flags (a list enclosed in " ")',
    default="",
)
parser.add_argument(
    "--extra-fortran-link-files",
    help='Extra Fortran link files (a list enclosed in " ")',
    default="",
)
...
L.basicConfig(
    level=getattr(L, args[0].log),
    format="%(levelname)s:%(message)s",
    filename=args[0].log_file,
)
L.debug(
    "setup.py called with the following optional args\n %s\n argument parsing completed.",
    vars(args[0]),
)
try:
...
if not (
    cython_version[0] > "0" or (cython_version[0] == "0" and cython_version[1] >= "18")
):
    msg = "Please upgrade to a newer Cython version, >= 0.18."
    L.error(msg)
    raise Exception(msg)
...
class Assimulo_prepare(object):
    ...
    def __init__(self, args, thirdparty_methods):
        ...
        if args[0].prefix:
            self.prefix = args[0].prefix.replace(
                "/", os.sep
            )  # required in this way for cygwin etc.
        ...
        self.BLASname_t = (
            args[0].blas_name
            if args[0].blas_name.startswith("lib")
            else "lib" + args[0].blas_name
        )
        ...
        if self.sundials_with_superlu is not None:
            L.warning(
                "The option 'sundials_with_superlu' has been deprecated and has no effect. Support for SuperLU using Sundials is automatically checked."
            )
    ...
    def _set_directories(self):
        ...
        self.desThirdParty = dict(
            [
                (thp, os.path.join(self.curdir, self.build_assimulo_thirdparty, thp))
        ...
        self.filelist_thirdparty = dict(
            [
                (thp, os.listdir(os.path.join("thirdparty", thp)))
                for thp in self.thirdparty_methods
            ]
        )
    ...
    def create_assimulo_dirs_and_populate(self):
        ...
        self.copy_all_files(
            self.fileSolvers, os.path.join("src", "solvers"), self.desSolvers
        )
        ...
        self.copy_all_files(
            self.fileTestsSolvers,
            os.path.join("tests", "solvers"),
            self.desTestsSolvers,
        )

        for f in self.filelist_thirdparty.items():
            L.debug("Thirdparty method {} file {} copied".format(f[0], f[1]))
            self.copy_all_files(
                f[1], os.path.join("thirdparty", f[0]), self.desThirdParty[f[0]]
            )
            try:
                SH.copy2(
                    os.path.join("thirdparty", f[0], "LICENSE_{}".format(f[0].upper())),
                    self.desLib,
                )
            except IOError:
                L.warning(
                    "No license file {} found.".format(
                        "LICENSE_{}".format(f[0].upper())
                    )
                )
        ...
        if self.extra_fortran_link_files:
            for extra_fortran_lib in self.extra_fortran_link_files:
                ...
                if path_extra_fortran_lib != None:
                    ...
                    L.debug(
                        "Could not find Fortran link file: " + str(extra_fortran_lib)
                    )
    ...
    def check_BLAS(self):
        ...
        if self.BLASdir == "":
            ...
            L.debug(
                "Note: the path required is to where the static library lib is found"
            )
            ...
            if not os.path.exists(os.path.join(self.BLASdir, self.BLASname_t + ".a")):
                ...
                L.debug(
                    "Could not find BLAS at the given path {}.".format(self.BLASdir)
                )
                L.debug("usage: --blas-home=path")
                self.with_BLAS = False
            else:
    ...
    def check_SuperLU(self):
        ...
        if self.SLUdir != "":
            ...
            if not os.path.exists(os.path.join(self.SLUincdir, "supermatrix.h")):
                self.with_SLU = False
                L.warning(
                    "Could not find SuperLU, disabling support. View more information using --log=DEBUG"
                )
                L.debug(
                    "Could not find SuperLU at the given path {}.".format(self.SLUdir)
                )
                L.debug("usage: --superlu-home path")
                L.debug(sundials_msg + ".")
            else:
                L.debug(
                    "SuperLU found in {} and {}: ".format(
                        self.SLUincdir, self.SLUlibdir
                    )
                )

            potential_files = [
                remove_prefix(f.rsplit(".", 1)[0], "lib")
                for f in listdir(self.SLUlibdir)
                if isfile(join(self.SLUlibdir, f)) and f.endswith(".a")
            ]
            ...
            L.debug(
                "No path to SuperLU supplied, SUNDIALS will not be compiled with support for SuperLU."
            )
            L.debug("usage: --superlu-home=path")
            L.debug(
                "Note: the path required is to the folder where the folders 'SRC' and 'lib' are found."
            )
            self.with_SLU = False
    ...
    def check_SUNDIALS(self):
        ...
        """
        if os.path.exists(
            os.path.join(os.path.join(self.incdirs, "cvodes"), "cvodes.h")
        ):
            ...
            try:
                if os.path.exists(
                    os.path.join(
                        os.path.join(self.incdirs, "sundials"), "sundials_config.h"
                    )
                ):
                    with open(
                        os.path.join(
                        ...
                        for line in f:
                            if (
                                "SUNDIALS_PACKAGE_VERSION" in line
                                or "SUNDIALS_VERSION" in line
                            ):
                                sundials_version = tuple(
                                    [int(f) for f in line.split()[-1][1:-1].split(".")]
                                )
                                L.debug(
                                    "SUNDIALS %d.%d found."
                                    % (sundials_version[0], sundials_version[1])
                                )
                                break
                    with open(
                        os.path.join(
                        ...
                        for line in f:
                            if "SUNDIALS_INT32_T" in line and line.startswith(
                                "#define"
                            ):
                                sundials_vector_type_size = "32"
                                L.debug(
                                    "SUNDIALS vector type size %s bit found."
                                    % (sundials_vector_type_size)
                                )
                                break
                            if "SUNDIALS_INT64_T" in line and line.startswith(
                                "#define"
                            ):
                                ...
                                if self.with_SLU:
                                    L.warning(
                                        "It is recommended to set the SUNDIALS_INDEX_TYPE to an 32bit integer when using SUNDIALS together with SuperLU (or make sure that SuperLU is configured to use the same int size)."
                                    )
                    ...
                    with open(
                        ...
                        for line in f:
                            if "SUNDIALS_SUPERLUMT" in line and line.startswith(
                                "#define"
                            ):  # Sundials compiled with support for SuperLU
                                sundials_with_superlu = True
                                L.debug(
                                    "SUNDIALS found to be compiled with support for SuperLU."
                                )
                                break
            except Exception as e:
                if os.path.exists(
                    os.path.join(os.path.join(self.incdirs, "arkode"), "arkode.h")
                ):  # This was added in 2.6
            ...
            if not self.sundials_with_superlu:
                L.debug(
                    "Could not detect SuperLU support with Sundials, disabling support for SuperLU."
                )
        else:
            L.warning(
                (
                    "Could not find Sundials, check the provided path (--sundials-home={}) "
                    + "to see that it actually points to Sundials."
                ).format(self.sundialsdir)
            )
            L.debug(
                "Could not find cvodes.h in " + os.path.join(self.incdirs, "cvodes")
            )
    ...
    def cython_extensionlists(self):
        ...
        ext_list = cythonize(
            ["assimulo" + os.path.sep + "*.pyx"], include_path=[".", "assimulo"]
        )
        # Cythonize Solvers
        # Euler
        ext_list += cythonize(
            ["assimulo" + os.path.sep + "solvers" + os.path.sep + "euler.pyx"],
        ...
        if self.with_SUNDIALS:
            ...
            if (
                self.sundials_with_superlu and self.with_SLU
            ):  # If SUNDIALS is compiled with support for SuperLU
                if self.SUNDIALS_version >= (3, 0, 0):
            ...
            # Kinsol
            ext_list += cythonize(
                ["assimulo" + os.path.sep + "solvers" + os.path.sep + "kinsol.pyx"],
                include_path=[".", "assimulo", "assimulo" + os.sep + "lib"],
                compile_time_env=compile_time_env,
                force=True,
            )
    ...
    def fortran_extensionlists(self):
        ...
        """
        extra_link_flags = (
            self.static_link_gfortran
            + self.static_link_gcc
            + self.flag_32bit
            + self.extra_fortran_link_flags
        )
        ...
        # to solve this issue --> https://github.com/JModelica/JModelica/issues/13
        ...
        # Hairer
        sources = (
            "assimulo" + os.sep + "thirdparty" + os.sep + "hairer" + os.sep + "{0}.f",
            "assimulo" + os.sep + "thirdparty" + os.sep + "hairer" + os.sep + "{0}.pyf",
        )
        config.add_extension(
            "assimulo.lib.dopri5",
            sources=[s.format("dopri5") for s in sources],
            **extraargs
        )
        config.add_extension(
            "assimulo.lib.rodas",
            sources=[s.format("rodas_decsol") for s in sources],
            include_dirs=[np.get_include()],
            **extraargs
        )
        config.add_extension(
            "assimulo.lib.radau5",
            sources=[s.format("radau_decsol") for s in sources],
            include_dirs=[np.get_include()],
            **extraargs
        )
        ...
        src = [
            "assimulo" + os.sep + "thirdparty" + os.sep + "hairer" + os.sep + code
            for code in radar_list
        ]
        config.add_extension(
            "assimulo.lib.radar5",
            sources=src,
        ...
        src = [
            "assimulo" + os.sep + "thirdparty" + os.sep + "odassl" + os.sep + code
            for code in odassl_list
        ]
        config.add_extension(
            "assimulo.lib.odassl",
            sources=src,
        ...
        src = [
            "assimulo" + os.sep + "thirdparty" + os.sep + "dasp3" + os.sep + code
            for code in dasp3_list
        ]
        config.add_extension(
            "assimulo.lib.dasp3dp",
            sources=src,
            include_dirs=[np.get_include()],
            extra_link_args=extra_link_flags[:],
            extra_f77_compile_args=dasp3_f77_compile_flags[:],
            extra_compile_args=extra_compile_flags[:],
            extra_f90_compile_args=extra_compile_flags[:],
        )
        ...
        if self.with_BLAS and self.with_LAPACK:
            ...
            src = [
                "assimulo" + os.sep + "thirdparty" + os.sep + "glimda" + os.sep + code
                for code in glimda_list
            ]
            extraargs_glimda = {
                "extra_link_args": extra_link_flags[:],
                "extra_compile_args": extra_compile_flags[:],
...
VERSION = "3.0" if version_number_arg == "Default" else version_number_arg
LICENSE = "LGPL"
...
license_info = [
    place + os.sep + pck + os.sep + "LICENSE_{}".format(pck.upper())
    for pck in thirdparty_methods
    for place in ["thirdparty", "lib"]
]
...
ndc.setup(
    ...
    package_data={
        "assimulo": ["version.txt", "CHANGELOG", "README", "LICENSE"]
        + license_info
        + [
        ...
        + (
            ["lib" + os.sep + f for f in prepare.extra_fortran_link_files]
            if prepare.extra_fortran_link_files
            else []
        )
    },
